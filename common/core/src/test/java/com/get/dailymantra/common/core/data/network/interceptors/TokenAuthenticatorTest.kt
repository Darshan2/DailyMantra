...
import com.get.dailymantra.common.core.data.rules.MainDispatcherRule
...
class TokenAuthenticatorTest {
    ...
    @Test
    fun `refresh failure clears tokens, and does not retry`() = runTest {
        val tokenProvider = FakeTokenProvider(accessToken = "old-access", refreshToken = "old-refresh")
    ...
    @Test
    fun `refresh failure notifies session expired`() = runTest {
        val tokenProvider = FakeTokenProvider(accessToken = "old-access", refreshToken = "old-refresh")
        val authApi = FakeAuthApi(Result.failure(IllegalStateException("refresh endpoint down")))
        val appEventBus = AppEventBus()
        ...
        appEventBus.events.test {
            ...
            val event = awaitItem()
            assertThat(event).isEqualTo(AppEvent.SessionExpired)
